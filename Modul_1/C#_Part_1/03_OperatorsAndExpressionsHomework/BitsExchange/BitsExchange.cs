...
namespace BitsExchange
{
    ...
    class BitsExchange
    {
        static void Main()
        {
            ...
            long shortBits = number >> 3;
            long numberShortBits = shortBits & 7;
            long longBits = number >> 24;
            long numberLongBits = longBits & 7;            

            long maskOne = ~(7 << 24);
            long shortAtLongChanger = numberShortBits << 24;
            long resultOne = (number & maskOne) | shortAtLongChanger;            

            long maskTwo = ~(7 << 3);
            long LongAtShortChanger = numberLongBits << 3;
